...
export const useOrganizationStore = create<OrganizationState>((set) => ({
  ...
  getOrganization: async (id:string) => {
    ...
    try {
      ...
      if (!response.data.success || !response.data.data)
        throw new Error(response.data.message)
      set({ currentOrganization: response.data.data, isLoading: false })
    } catch (error: any) {
      set({ error: error.response?.data?.message ?? error.message, isLoading: false })
      throw error
    }
  ...
  createOrganization: async (data) => {
    ...
    try {
      ...
      set((state) => ({ 
        ...
        userOrganizations: response.data.data ? [...state.userOrganizations, response.data.data] : state.userOrganizations,
        error: null,
        isUpdated: false,
        isDeleted: false,
      }))
  ...
  updateOrganization: async (id, data) => {
    ...
    try {
      ...
      if (!response.data.success)
        throw new Error(response.data.message)
      set((state) => ({
        userOrganizations: state.userOrganizations.map(o => o.id === id ? { ...o, ...data } : o),
        isLoading: false,
        isUpdated: true
      }))
    } catch (error: any) {
